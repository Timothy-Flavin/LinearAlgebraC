...
void g_elim(int rows, int l_cols, int r_cols, double* mat){
  if(l_cols>rows){
  ...
  for(int c=0; c<l_cols-1; c++){
    ...
    while(r<rows && pivot<0){
      printf("Testing %i,%i for pivot: %.3f\n",r,c,mat[r*cols+c]);
      if(mat[r*cols+c]!=0.0){
        pivot = r;
        printf("%i Pivot row found\n",pivot);
      }
      r++;
    }
    if(pivot==-1)
      return;
    if(pivot!=c){
      printf("pivot: r%i != c: r%i, so swapping pivot and c\n",pivot,c);
      swap_r(pivot,c,mat,l_cols,r_cols);
      print_mat(rows, l_cols, r_cols, mat);
    }
    for(r=c+1; r<rows; ++r){
      double a = -(mat[r*cols+c]/mat[c*cols+c]);
      printf("adding %.3f*r%i to r%i\n",a,c,r);
      add_r(c, r, a, mat, l_cols, r_cols);
      print_mat(rows, l_cols, r_cols, mat);
    }
    printf("Scaling pivot row to 1 p: %.3f, c: %.3f, a: %.3f\n",pivot,c, 1/mat[c*cols+c]);
  ...
  for(int c=l_cols-1; c>0; --c){
    if(mat[c*cols+c]==0){
      printf("r%i diagonal is zero. skipping it...");
      continue;
    }
    for(int r=c-1; r>-1; --r){
      double a = -(mat[r*cols+c]/mat[c*cols+c]);
...
int main(int argc, char** argv) {
  ...
  //for(int i=0; i<12; ++i){
    //mat[i]=i+1;
  //}
  double m[12] = {0,1,-1,3,-2,4,-1,1,-2,5,-4,-2};
  //mat=m;
  //mat[0]=4.0;
  g_elim(r, 3, 1, m);
}